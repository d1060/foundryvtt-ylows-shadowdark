...
export default function listenOnSocket() {
	...
	game.socket.on("system.shadowdark", event => {
		...
		if (event.type === "toggleLightSource") {
			game.shadowdark.lightSourceTracker.externalToggleLightSource(
				event.data.actor,
				event.data.item
			);
		}
		...
		if (event.type === "changeNpcActorHP" && game.user.isGM) {
			...
			CONFIG.DiceSD.applyDamageToToken(token._object, parseInt(event.data.damage), event.data.hitLocation, event.data.damageType);
		}
